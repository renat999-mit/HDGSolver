...
class Master1D:
    def __init__(self, mesh):
        self.porder = mesh.porder
        self.plocal = mesh.plocal
        self.extremes = np.zeros((2,), dtype=int)
...
def mkmaster1D(mesh, pgauss=None):
    """         
    mkmaster  initialize master element structures
    master=mkmaster(mesh)
    ...
    master = Master1D(mesh)
    
    master.extremes[0] = 0
    master.extremes[1] = len(master.plocal)
    ...
    master.sh1d = shape1d(master.porder, master.plocal, master.gp1d)
...
def tlocal1d(porder):
    ...
    tlocal = np.zeros((porder-1,2), dtype = int)
    
    for i in range(len(tlocal)):
        
        tlocal[i,:] = np.array([i,i+1])
...
def shape1d(porder, plocal, pts):
    ...
    if len(plocal.shape) == 1:
        
        A, _ = koornwinder1d(plocal, porder)
