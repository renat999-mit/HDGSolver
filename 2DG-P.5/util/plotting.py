...
def plot_1D(mesh,uh,ax,linestyle = '-', label = None):
    
    nt = len(mesh.t)
    npl = len(mesh.dgnodes[:,0,0])
    
    xvec = np.zeros((nt*npl))
    uvec = np.zeros((nt*npl))
    
    for e in range(nt):
        
        indices = np.arange(e*npl,(e+1)*npl)
        
        xvec[indices] = mesh.dgnodes[:,0,e]
        uvec[indices] = uh[:,e]
    
    ax.plot(xvec,uvec, linestyle = f'{linestyle}', label = f'{label}')
    
def plot_1D_uhat(mesh,uhat,ax):
    ...
    for e in range(nt):
        ...
        uvec[e:e+2] = uhat[:,e]
        xvec[e:e+2] = np.squeeze(mesh.p[mesh.t[e]])
        
    ax.scatter(xvec,uvec)
