...
class Mesh1D:
    def __init__(self, p, t, f, porder, plocal, tlocal):
        self.p = p
        self.t = t
        self.f = f
        self.porder = porder
...
def createnodes_1D(mesh):
    ...
    dp = dp_element/(len(plocal)-1)

    # Allocte nodes
    dgnodes = np.zeros((plocal.shape[0], 1, t.shape[0]), dtype=float)
    
    for nt in range(len(t)):
        for p in range(len(plocal)):
            
            dgnodes[p,0,nt] = p*dp
            
        dgnodes[:,0,nt] += nt*dp_element
...
def interval_1D(m):
    ...
    # Generate mesh for unit interval (1D)
    
    m = int(m)

    # x = np.meshgrid(np.linspace(0.0, 1.0, m))
    x = np.linspace(0.0, 1.0, m)
    p = np.vstack([x.flatten()]).T

    t = np.zeros((m-1,2), dtype = int)
